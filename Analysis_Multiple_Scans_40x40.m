...
for sensorNum=1:length(filesList)
    cd (fileString)
    close all
    clearvars -except filesList sensorNum noiseThreshold fileString sqEdge
    addpath 'H:\Documents\Hexitec\Matlab'
    ...
    pixelHist=zeros(length(edges),sqEdge,sqEdge);
    pixelHistCSA=zeros(length(edges),sqEdge,sqEdge);
    pixelHistCSD=zeros(length(edges),sqEdge,sqEdge);
    noiseHist=zeros(length(edges),sqEdge,sqEdge);
    noiseMultiplier=8;
    ...
    for fNum = 1:10000%(nFrames/2)-1
        ...
        frame40 = zeros(40,40);
        [xLocs,yLocs]=find(frame>30);
        yLocs40=ones(1,length(xLocs));
        xLocs40=ones(1,length(xLocs));
        for n=1:length(xLocs)
            if yLocs(n)>1
               yLocs40(n)=ceil(yLocs(n)/2);
            end
            if xLocs(n)>1
               xLocs40(n)=ceil(xLocs(n)/2);
            end   
            frame40(xLocs40(n),yLocs40(n)) = frame(xLocs(n),yLocs(n));
        end
    ...
    for fNum = 1:(nFrames/4)-1
        ...
        frameCSD = zeros(sqEdge,sqEdge);
        frameCSA = zeros(sqEdge,sqEdge);
...
% pause
    ...
    peakAmplitudes = zeros(sqEdge,sqEdge,3);
    peakCentroids = zeros(sqEdge,sqEdge,3);
    m = zeros(sqEdge,sqEdge);
    ...
    %% Data Viulisation
    figureFile = strcat('Figures_', sensorName, '40x40');
    mkdir(figureFile)
    cd(figureFile)
...
%     saveas(fig,'Noise.fig')
    ...
    edges=binMin:binWidth:binMax;
    calibratedEdges = zeros(length(edges),sqEdge,sqEdge);
    x=0; y=1;
    for z = 1:(sqEdge*sqEdge)
        x=x+1;
        edgesCopy=edges;
    ...
    %% Peaks analysis
    peakCounts = zeros(sqEdge,sqEdge);
    peakAmp = zeros(sqEdge,sqEdge);
    peakCentADU = zeros(sqEdge,sqEdge);
    peakCentkeV = zeros(sqEdge,sqEdge);
    peakFWHM = zeros(sqEdge,sqEdge);
